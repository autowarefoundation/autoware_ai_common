...
public:
  PreProcessorYoloV2Tiny(tvm_utility::pipeline::InferenceEngineTVMConfig config)
    ...
    std::vector<int64_t> shape_x{1, network_input_width, network_input_height,
                                 network_input_depth};
...
public:
  ...
  schedule(const tvm_utility::pipeline::TVMArrayContainerVector &input) {
    ...
    float *data_ptr = (float *)((uint8_t *)input[0].getArray()->data +
...
TEST(PipelineExamples, SimplePipeline) {
  ...
  Pipeline<PreProcessorYoloV2Tiny, InferenceEngineTVM, PostProcessorYoloV2Tiny>
      pipeline(PreProcessorYoloV2Tiny{model_config::config},
               ...
               PostProcessorYoloV2Tiny{model_config::config});
  ...
  std::vector<float> expected_output{0.360896, 0.763201, 0.402838, 0.302775,
                                     0.612508, 0.583656, 0.422576, 0.452684,
                                     0.372608, 0.806072, 0.572778};
