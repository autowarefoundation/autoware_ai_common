...
TEST_F(TestSuite, MatchWaypointAndLanelet)
{
  ...
  // waypoint that overlaps with road_lanelet
  waypoint.gid = 1;
  waypoint.pose.pose.position.x = 0.5;
  waypoint.pose.pose.position.y = 0.5;
  ...
  ASSERT_EQ(1, waypointid2laneletid.size()) << "failed to match waypoints with lanelets";

  ASSERT_EQ(road_lanelet.id(), waypointid2laneletid.at(1)) << "failed to match waypoint with lanelet";
}
